...
ablate_path = "/work/koichi/Gear/gear_gen_angle_rad_con_auto/eval_results/Gen_20/Gear_Gen_Angle_Rad_Con_LowNoiseAutoGen_SCRATCH_DefaultWeight_NoTime_NoInterval_8000_step32b/"
    ...
    for loop in range(loop_max + 1):
        if loop == loop_max:
            ...
            example = [
                ...
                (f"Generated (Default Schedule)", os.path.join(ablate_path, "results", f"{g}_loop_{loop}_modality_0.mp4")),
                (f"Generated (Our Schedule)", os.path.join(base_path, "results", f"{g}_loop_{loop}_modality_0.mp4")),
            ]
        lis.append([example])
    examples.append(lis)
...
def visualize(grid_inputs, output_path):
    ...
    try:
        ...
        border_cfg = {
            ...
            "Generated (Default Schedule)": (128, 128, 128), #Light Gray border
            "Generated (Our Schedule)": (255, 0, 0)    # Red border
        }
...
if __name__ == "__main__":
    output_dir = "./../../video/Ablation: Autoregressive Design/"
